...
import { DEFAULT_PROFILE_SELF_ID, type OwnershipProfile, type OwnershipSplit } from '@/types/household';
...
import { getAssignableOwnershipProfiles, getDefaultProfile, profileToAssignment } from '@/lib/utils/householdUtils';
...
function normalizeOptionalLinkedInvestmentQuantityDelta(value: unknown): number | undefined {
  return normalizeOptionalLinkedInvestmentNumber(value);
}

function getInvalidSubmitMessage(
  ...
  const invalidFieldMessages: Partial<Record<keyof ExpenseFormValues, string>> = {
    type: 'Seleziona un tipo di voce valido',
    categoryId: 'Seleziona una categoria',
    amount: 'Inserisci un importo valido',
    currency: 'Seleziona una valuta valida',
    date: 'Inserisci una data valida',
    link: 'Inserisci un URL valido',
    installmentTotalAmount: 'Inserisci un importo totale valido',
    installmentCount: 'Inserisci un numero di rate valido',
    recurringDay: 'Inserisci un giorno del mese valido',
    recurringMonths: 'Inserisci un numero di mesi valido',
    attributionProfileId: 'Seleziona un profilo di attribuzione',
    linkedCashAssetId:
      selectedType === 'transfer'
        ? 'Seleziona il conto di origine'
        : 'Seleziona un conto valido',
    transferCashAssetId: 'Seleziona il conto di destinazione',
  };

  const fieldName = Object.keys(errors)[0] as keyof ExpenseFormValues;
  const errorMessage = fieldName ? String(errors[fieldName]?.message ?? '') : '';
  ...
  return invalidFieldMessages[fieldName] ?? errorMessage;
}
...
interface FormBodyProps {
  ...
  onInvalidSubmit: (errors: FieldErrors<ExpenseFormValues>) => void;
  isEdit: boolean;
  selectedType: ExpenseType;
  ...
  watchedAttributionProfileId: string | undefined;
  ...
  householdProfiles: OwnershipProfile[];
  defaultExpenseAttributionProfileId: string;
  defaultIncomeAttributionProfileId: string;
...
function ExpenseFormBody({
  ...
  return (
    <form id="expense-form" noValidate onSubmit={handleSubmit(onSubmit, onInvalidSubmit)} className="space-y-5">
      ...
      <input type="hidden" {...register('linkedInvestmentAssetId')} />
      <input type="hidden" {...register('linkedInvestmentAssetName')} />
      <input type="hidden" {...register('linkedInvestmentQuantityDelta', { valueAsNumber: true })} />
      <input type="hidden" {...register('investmentOperationId')} />
      <input type="hidden" {...register('investmentOperationType')} />
      <input type="hidden" {...register('investmentOperationPricePerUnit', { valueAsNumber: true })} />
      <input type="hidden" {...register('investmentOperationFees', { valueAsNumber: true })} />
      <input type="hidden" {...register('investmentOperationTaxes', { valueAsNumber: true })} />
      ...
      <div className="space-y-2">
        <Label htmlFor="attributionProfileId">Attribuzione</Label>
        <Select
          value={watchedAttributionProfileId || DEFAULT_PROFILE_SELF_ID}
          onValueChange={(value) => setValue('attributionProfileId', value)}
        >
...
export function ExpenseDialog({ open, onClose, expense, onSuccess }: Readonly<ExpenseDialogProps>) {
  ...
  const householdProfiles = useMemo(() => getAssignableOwnershipProfiles(householdConfig), [householdConfig]);
  const defaultExpenseAttributionProfileId =
    householdConfig?.defaultExpenseProfileId || DEFAULT_PROFILE_SELF_ID;
  const defaultIncomeAttributionProfileId =
  ...
  const form = useForm<ExpenseFormValues>({
    ...
    defaultValues: {
      ...
      investmentOperationType: undefined,
  ...
  const { reset, setValue, getValues, control, handleSubmit, formState: { isSubmitting } } = form;
  ...
  useEffect(() => {
    ...
    if (expense) {
      reset({
        ...
        installmentMode: 'auto',
        installmentCount: expense.installmentTotal || 2,
        installmentTotalAmount: expense.installmentTotalAmount || Math.abs(expense.amount),
        installmentAmounts: [],
        installmentStartDate: expense.date,
        ...
        linkedInvestmentAssetName: typeof expense.linkedInvestmentAssetName === 'string' ? expense.linkedInvestmentAssetName : '',
        linkedInvestmentQuantityDelta:
          normalizeOptionalLinkedInvestmentQuantityDelta(expense.linkedInvestmentQuantityDelta),
        investmentOperationId: expense.investmentOperationId || '',
        investmentOperationType: expense.investmentOperationType || undefined,
        ...
        attributionProfileId: expense.attributionProfileId || (expense.type === 'income' ? defaultIncomeAttributionProfileId : defaultExpenseAttributionProfileId),
        attributionProfileName: expense.attributionProfileName || '',
      });
  ...
  }, [open, expense, resetKey, reset, defaultExpenseAttributionProfileId, defaultIncomeAttributionProfileId]);
  ...
  }, [open, expense, selectedType, defaultExpenseAttributionProfileId, defaultIncomeAttributionProfileId, setValue]);
  ...
  const onSubmit = async (data: ExpenseFormValues) => {
    ...
    const linkedInvestmentAssetName =
      watchedLinkedInvestmentAssetName?.trim().length ? watchedLinkedInvestmentAssetName.trim() : undefined;
    const investmentOperationPricePerUnit = watchedInvestmentOperationPricePerUnit;
    const attributionProfile =
      householdProfiles.find((profile) => profile.id === (data.attributionProfileId || watchedAttributionProfileId)) ??
      getDefaultProfile(householdConfig, data.attributionProfileId || watchedAttributionProfileId);
    const attributionAssignment = profileToAssignment(attributionProfile, data.date);
    ...
    try {
      const expenseData: ExpenseFormData = {
        ...
        investmentOperationId: data.investmentOperationId || undefined,
        investmentOperationType: data.investmentOperationType || undefined,
        investmentOperationPricePerUnit,
        investmentOperationFees: normalizeOptionalLinkedInvestmentNumber(data.investmentOperationFees),
        investmentOperationTaxes: normalizeOptionalLinkedInvestmentNumber(data.investmentOperationTaxes),
  ...
  const onInvalidSubmit = (errors: FieldErrors<ExpenseFormValues>) => {
    toast.error(getInvalidSubmitMessage(errors, selectedType));
  };
    ...
    <>
      <ResponsiveModal
        ...
        footer={
          isMobile ? (
            <>
              <Button type="button" onClick={handleSubmit(onSubmit, onInvalidSubmit)} disabled={isSubmitting} className="w-full">
                {submitLabel}
              </Button>
