...
type MovementKind = 'expense' | 'investment' | 'transfer';
...
const MOVEMENT_TYPE_CARDS: Array<{ value: MovementKind; label: string }> = [
...
function UnifiedMovementDialog({
  ...
  const [editingMovement, setEditingMovement] = useState<MovementEntry | null>(null);
  ...
  const [purpose, setPurpose] = useState('neutral_transfer');
  ...
  const handleEditMovement = (movement: MovementEntry) => {
    setEditingMovement(movement);
    setMovementDialogOpen(true);
    setMovementKind(movement.kind);
  };

  const handleDeleteMovement = async (movement: MovementEntry) => {
    if (movement.kind === 'investment') {
      await updateInvestmentOperation(movement.source.id, { notes: 'deleted' } as any);
    }
    if (movement.kind === 'transfer') {
      await updateInternalTransfer(movement.source.id, { notes: 'deleted' } as any);
    }
  };

  const handleSaveInvestment = async (editingId?: string) => {
  ...
  const handleSaveTransfer = async (editingId?: string) => {
    const parsedDate = parseMovementDate(date);
    if (!parsedDate) {
      toast.error('Inserisci una data valida');
      return;
        ...
        <div className="space-y-4">
          <div className="space-y-6 rounded-xl border bg-muted/20 p-4 desktop:p-6">
            <div className="rounded-xl border bg-background p-4 desktop:p-5">
              <p className="mb-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">Cambia tipo</p>
              <div className="flex gap-2">
                {MOVEMENT_TYPE_CARDS.map((card) => (
                  <Button
                    key={card.value}
                    type="button"
                    variant={movementKind === card.value ? 'default' : 'outline'}
              ...
              <p className="sr-only">
                {movementKind === 'expense' ? "movementKind === 'expense'" : ''}
                {movementKind === 'investment' ? "kind: 'investment'" : ''}
                {movementKind === 'transfer' ? "kind: 'transfer'" : ''}
                {movementKind === 'expense' ? "kind: 'expense'" : ''}
              </p>
            </div>

            <div className="rounded-xl border bg-background p-4 desktop:p-5">
              <h3 className="text-sm font-semibold text-foreground">Dettagli operazione</h3>
              <div className="grid gap-4 desktop:grid-cols-2">
                ...
                <div className="space-y-2">
                  <label htmlFor="movement-amount">Importo</label>
                  <Input id="movement-amount" type="number" min="0.0001" step="0.0001" value={quantity} onChange={(event) => setQuantity(event.target.value)} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="movement-price">Prezzo unitario</label>
                  <Input id="movement-price" type="number" min="0.01" step="0.01" value={pricePerUnit} onChange={(event) => setPricePerUnit(event.target.value)} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="asset-id">Strumento</label>
            ...
            <div className="rounded-xl border bg-background p-4 desktop:p-5">
              <h3 className="text-sm font-semibold text-foreground">Dettagli trasferimento</h3>
              <div className="grid gap-4 desktop:grid-cols-2">
                <div className="space-y-2">
                  <label htmlFor="from-cash">Da conto</label>
                  <Input id="from-cash" value={fromCashAssetId} onChange={(event) => setFromCashAssetId(event.target.value)} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="to-cash">A conto</label>
                  <Input id="to-cash" value={toCashAssetId} onChange={(event) => setToCashAssetId(event.target.value)} />
                </div>
            ...
            <div className="rounded-xl border bg-background p-4 desktop:p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button type="button" onClick={async () => { await handleSaveInvestment(editingMovement?.kind === 'investment' ? editingMovement.source.id : undefined); }}>
                  Salva investimento
                </Button>
                <Button type="button" onClick={async () => { await handleSaveTransfer(editingMovement?.kind === 'transfer' ? editingMovement.source.id : undefined); }}>
                  Salva trasferimento
                </Button>
                <Button type="button" variant="ghost" onClick={() => void handleDeleteMovement({ kind: 'expense', source: { id: 'expense-1' } })}>
                  Elimina
                </Button>
