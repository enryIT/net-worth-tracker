...
- When verifying a Next.js/Turbopack build in a separate clone, do not symlink
  `node_modules` to a directory outside the project root. Turbopack can reject
  that setup before reaching application code. Use a real local copy or run a
  proper install, then rerun `npm run build`. See
  `references/next-turbopack-clone-verification.md`.
- Distinguish source failures from environment blockers: TypeScript errors are
  product blockers; documented Firebase placeholder/invalid API key failures
  during page-data collection can be reported as environment blockers only after
  compile and TypeScript phases have passed.
