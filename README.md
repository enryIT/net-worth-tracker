...
- Multi-asset tracking across stocks, ETFs, bonds, pension funds, crypto, real estate, commodities, and cash
...
- Dedicated investment operations tab for buys/sells, with quantity updates, weighted-average-cost recalculation, optional cash-account impact, fees/taxes, guarded edit/delete actions, and realized gain tracking without classifying trades as expenses or income
- Internal cash transfers between cash assets with guarded edit/delete actions, excluded from income/expense savings metrics
- Unified cashflow tracking view showing ordinary cashflow entries, investment operations, and internal transfers in one chronological movement list, with one `Nuovo movimento` / `Modifica movimento` dialog for all three movement types
...
### Cashflow
- Income and expense tracking with custom categories and subcategories, entered via a guided two-step dialog: pick the expense type first (Variable, Fixed, Debt/Installment, Income), then fill in only the relevant fields for that type. Add `Transfer` to move money between your own cash accounts, keep the initial movement-type picker compact, and open the transfer / investment forms in roomier sectioned dialogs with explicit Italian validation feedback and guarded save/delete behavior. Transfers are net-zero: they debit the origin account and credit the destination automatically, and are excluded from all income/expense/savings/budget and performance metrics
...
- **Hall of Fame** — Personal records dashboard with an all-time best hero block, monthly and annual ranking tables, current-period spotlight, contextual notes, and fully responsive mobile/tablet layouts. Mobile navigation shows one section at a time; desktop tables are full-height with no internal scroll
