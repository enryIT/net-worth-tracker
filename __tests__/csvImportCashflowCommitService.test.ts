...
type MockBatchRepository = CsvImportCashflowBatchRepository & {
  ...
  getDividendById(dividendId: string): MockDividendRecord | null;
  getDividendCount(): number;
  listDividendsByBatchId(batchId: string): Promise<MockDividendRecord[]>;
  mutateDividend(dividendId: string, patch: Partial<MockDividendRecord>): void;
...
function createBatchRepository(
  ...
  const dividends = new Map<string, MockDividendRecord>();
  ...
  function applyDividendRecord(record: MockDividendRecord): void {
    dividends.set(record.id, record);
  }

  function reverseDividendRecord(record: MockDividendRecord): void {
    dividends.delete(record.id);
  }
  ...
  return {
    ...
    async commitBatch(
      ...
      createdDividends: MockDividendRecord[] = []
    ) {
      ...
      createdDividends.forEach((record) => {
        applyDividendRecord(record);
      });
    },
    ...
    async listDividendsByBatchId(batchId: string) {
      return Array.from(dividends.values())
        .filter((dividend) => dividend.importBatchId === batchId)
        .sort((left, right) => left.rowIndex - right.rowIndex);
    },
    ...
    async rollbackBatch(
      ...
      dividendIdsOrRolledBackAt: string[] | Date,
      rolledBackAtOrRollbackReason?: Date | string,
      rollbackReason?: string
    ) {
      const dividendIds = Array.isArray(dividendIdsOrRolledBackAt)
        ? dividendIdsOrRolledBackAt
        : [];
      const rolledBackAt = Array.isArray(dividendIdsOrRolledBackAt)
        ? rolledBackAtOrRollbackReason as Date
        : dividendIdsOrRolledBackAt;
      const resolvedRollbackReason = Array.isArray(dividendIdsOrRolledBackAt)
        ? rollbackReason ?? 'annullamento manuale'
        : rolledBackAtOrRollbackReason as string;
      ...
      dividendIds
        .map((dividendId) => dividends.get(dividendId))
        .filter((dividend): dividend is MockDividendRecord => Boolean(dividend))
        .sort((left, right) => left.rowIndex - right.rowIndex)
        .reverse()
        .forEach((dividend) => {
          reverseDividendRecord(dividend);
        });
      ...
      const updatedBatch = {
        ...
        rolledBackAt,
        rollbackReason: resolvedRollbackReason,
      };
    ...
    getDividendById(dividendId: string) {
      return dividends.get(dividendId) ?? null;
    },
    getDividendCount() {
    ...
    mutateDividend(dividendId: string, patch: Partial<MockDividendRecord>) {
      const current = dividends.get(dividendId);
      if (!current) return;
      dividends.set(dividendId, { ...current, ...patch });
    },
...
function createDividendRow(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    rowIndex: 1,
    movementKind: 'dividend' as const,
    ready: true,
    issues: [],
...
function toCommitRows(rows: CsvImportCashflowCommitRowInput[]): CsvImportCashflowCommitRowInput[] {
  return rows;
}
...
describe('csv import cashflow commit service', () => {
  ...
  it('commits ready dividend rows and standalone fee/tax rows as tracked import records', async () => {
    const repository = createBatchRepository([], [], [], [
      createInvestmentAsset({
        id: 'bond-1',
        userId: 'user-1',
    ...
    const {
      ...
      categoryRepository: createCategoryRepository([
        {
          id: 'expense-investment-fees',
          userId: 'user-1',
          name: 'Commissioni investimento',
          type: 'variable',
          subCategories: [],
        },
      ]),
      now: () => new Date('2026-06-03T10:00:00.000Z'),
      generateId: (() => {
        const ids = ['batch-1', 'dividend-1', 'expense-1', 'expense-2'];
        return () => ids.shift() ?? 'fallback-id';
      })(),
    });
    ...
    expect(committed.batch.createdRecords).toEqual([
      expect.objectContaining({
        kind: 'dividend',
        id: 'dividend-1',
        rowIndex: 1,
    ...
    expect(repository.getDividendCount()).toBe(1);
    expect(repository.getDividendById('dividend-1')).toMatchObject({
      importBatchId: 'batch-1',
      importIdempotencyKey: 'idempotency-dividend-1',
      dividendType: 'coupon',
      costPerShare: 98.5,
      grossAmount: 100,
      taxAmount: 26,
    ...
    expect(repository.getExpenseCount()).toBe(2);
    expect(repository.getExpenseById('expense-1')).toMatchObject({
      importBatchId: 'batch-1',
      importIdempotencyKey: 'idempotency-dividend-1',
      amount: -12.5,
      type: 'variable',
    });
    expect(repository.getExpenseById('expense-2')).toMatchObject({
      importBatchId: 'batch-1',
      importIdempotencyKey: 'idempotency-dividend-1',
      amount: -26,
      type: 'variable',
    ...
    const rollback = await service.rollbackBatch('user-1', 'batch-1', 'annullamento cedola');
    expect(rollback).toMatchObject({
      batch: {
        id: 'batch-1',
        status: 'rolledBack',
        rollbackReason: 'annullamento cedola',
      },
      removedRecordCount: 3,
    });
    expect(repository.getDividendCount()).toBe(0);
    expect(repository.getExpenseCount()).toBe(0);
  ...
  it('blocks rollback when an imported dividend row was manually edited after commit', async () => {
    const repository = createBatchRepository([], [], [], [
      createInvestmentAsset({
        id: 'bond-1',
        userId: 'user-1',
    ...
    const {
      ...
      now: () => new Date('2026-06-03T10:00:00.000Z'),
      generateId: (() => {
        const ids = ['batch-1', 'dividend-1'];
        return () => ids.shift() ?? 'fallback-id';
      })(),
    });

    const committed = await service.commitBatch('user-1', {
      idempotencyKey: 'idempotency-dividend-unsafe',
      rows: toCommitRows([
        createDividendRow(),
      ]),
    });

    repository.mutateDividend('dividend-1', {
  ...
  it('returns 404 when the confirmed asset reference does not resolve to an authenticated user asset', async () => {
    const {
      service,
    } = createStubbedCashflowCommitService({
    ...
    await expect(
      service.commitBatch('user-1', {
        idempotencyKey: 'idempotency-dividend-ownership',
        rows: toCommitRows([
          createDividendRow({
            canonicalFields: {
              ...createDividendRow().canonicalFields,
              assetTicker: 'BTP',
              assetIsin: 'IT0000000001',
              assetName: 'BTP 5%',
            },
