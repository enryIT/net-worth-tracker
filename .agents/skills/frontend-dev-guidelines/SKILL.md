...
### UI Workflow Consolidation

When collapsing multiple visible UI flows into one user workflow (for example separate tabs/forms becoming one `Nuovo movimento` entrypoint), follow the reference checklist in `references/ui-workflow-consolidation.md`.

Required pattern:

1. Add a RED regression test first that proves the old split entrypoints/forms are still present or the new unified entrypoint is missing.
2. Define a small UI-level discriminated model (`kind` plus shared list fields) while preserving type-specific source objects and fields.
3. Use one visible entrypoint and one visible list; route type-specific create/update/delete actions to existing domain services rather than rewriting persistence in the UI.
4. Remove old embedded or duplicate forms from the page if the requested outcome is true UX consolidation.
5. Be explicit if the implementation delegates internally to an existing complex dialog for one branch; do not overclaim a single physical form component unless it is actually true.

### Edit-Mode Form State Reset

When debugging a form submit button that appears to do nothing in edit mode, check hidden or previously-mounted form state before assuming the button handler is broken. Libraries such as React Hook Form keep registered field values even when their UI is hidden by conditional rendering; stale values can fail schema validation before `onSubmit` runs, producing a silent no-op from the user's perspective. Add a RED regression test, then ensure edit-mode `reset()` includes all conditional flags and companion fields with safe defaults. See `references/form-edit-state-reset.md`.
...
When merging separate tabs, menus, forms, or lists into a single workflow, preserve behavior by introducing a parent-level unified model before rewriting mature specialist forms. Use a discriminated union for the shared list model, keep type-specific fields conditional, and add RED regression tests that prove old entrypoints are gone while the new entrypoint and variant-specific fields remain reachable. Revert transitional child-component edits that become unused before staging. See `references/ui-consolidation-refactors.md` for the detailed checklist.
