...
import { getLocalYocMetrics } from "@/lib/server/performance/localYieldMetricsService";
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      await getLocalYocMetrics(user.id, periodQuery.value)
    );
  } catch (error) {
    return handlePerformanceYieldRouteError(error, "[LOCAL_PERFORMANCE_YOC_ERROR]");
  }
}
