 ...
 */
export function budgetItemKey(
  item: Pick<BudgetItem, 'scope' | 'expenseType' | 'categoryId' | 'subCategoryId' | 'attributionProfileId'>
): string {
...
function inferBudgetKind(item: Partial<BudgetItem>): 'expense' | 'income' {
  if (item.kind === 'expense' || item.kind === 'income') return item.kind;
  return item.scope === 'type' && item.expenseType === 'income' ? 'income' : 'expense';
}
...
function expenseMatchesItem(expense: Expense, item: BudgetItem): boolean {
  ...
  switch (item.scope) {
    ...
    case 'subcategory':
      return expense.categoryId === item.categoryId && expense.subCategoryId === item.subCategoryId;
    default:
      return false;
  }
}
 ...
 */
export function getActualForItem(item: BudgetItem, expenses: Expense[], year: number): number {
  let total = 0;
 ...
 */
export function getMonthlyActualsForItem(item: BudgetItem, expenses: Expense[], year: number): number[] {
 ...
 */
export function getMonthlyTotalExpenses(expenses: Expense[], year: number, month: number): number {
  let total = 0;
  for (const expense of expenses) {
...
export function getMonthActualForItem(item: BudgetItem, expenses: Expense[], year: number, month: number): number {
  let total = 0;
  for (const expense of expenses) {
    const { month: expMonth, year: expYear } = getItalyMonthYear(toDate(expense.date));
    if (expYear !== year || expMonth !== month) continue;
 ...
 */
export function getPeriodExpensesForItem(item: BudgetItem, expenses: Expense[], now: Date = new Date()): Expense[] {
  const year = getItalyYear(now);
...
export function getDefaultAmount(
  item: Partial<BudgetItem> & Pick<BudgetItem, 'scope'>,
  expenses: Expense[],
...
/**
 * Backwards-compatible helper used by the budget tab to prefill monthly amounts.
 */
export function getDefaultMonthlyAmount(
  ...
  return getDefaultAmount(item, expenses, historyStartYear, 'monthly');
}
 ...
 */
export function reconcileBudgetItems(categories: ExpenseCategory[], existingItems: BudgetItem[]): BudgetItem[] {
 ...
 *
 * Strategy:
 *   1. One category-scope item per category, grouped by expense type
 *      (fixed → variable → debt → income) and sorted alphabetically within each group.
 *   2. Amounts are taken from existingItems when available (preserves user customizations),
 *      otherwise pre-filled via getDefaultMonthlyAmount.
...
export function autoInitBudgetItems(
  ...
  const sortedCategories = [...categories]
    ...
    .sort((a, b) => {
      const typeDiff = (SECTION_ORDER[a.type] ?? 9) - (SECTION_ORDER[b.type] ?? 9);
  ...
  const categoryItems: BudgetItem[] = sortedCategories.map((category) => {
    const key = budgetItemKey({ scope: 'category', categoryId: category.id });
    const existing = existingByKey.get(key);
    const sectionOrder = orderCounter[category.type] ?? 0;
    orderCounter[category.type] = sectionOrder + 1;

    const amount =
      existing?.amount ??
      getDefaultMonthlyAmount({ scope: 'category', categoryId: category.id, categoryName: category.name }, expenses, historyStartYear);
    ...
    return {
      ...
      categoryName: category.name,
      amount,
      attributionProfileId: existing?.attributionProfileId,
      attributionProfileName: existing?.attributionProfileName,
      attributionSplits: existing?.attributionSplits,
      order: sectionOrder,
    };
  });
  ...
  const subItems = existingItems
    .filter((item) => item.scope === 'subcategory' && item.categoryId != null && validCategoryIds.has(item.categoryId))
    .map((item) => {
      const category = categories.find((c) => c.id === item.categoryId);
      const sub = category?.subCategories.find((s) => s.id === item.subCategoryId);
      return {
        ...item,
        kind: category ? categoryKind(category) : item.kind,
        categoryName: category?.name ?? item.categoryName,
        subCategoryName: sub?.name ?? item.subCategoryName,
        order: item.order ?? 0,
      };
    });
...
export function evaluateBudgetAlerts(
  ...
  if (overallMonthlyAmount && overallMonthlyAmount > 0) {
    ...
    const overrun =
      canForecast &&
      buildSpendingForecast(overallSpent, overallMonthlyAmount, now, reference).projectedTotal > overallMonthlyAmount;
...
export function sectionWeight(item: BudgetItem, categories: ExpenseCategory[]): number {
  if (item.kind === 'income' || item.expenseType === 'income') return SECTION_ORDER.income;
